 ...
 * parallel across cores). Protocol: receive `{ id, path, maxPages }`, reply
 * `{ id, text }`; extraction never throws (empty string on any failure).
 */
...
parentPort?.on('message', (msg: { id: number; path: string; maxPages?: number }) => {
  void extractPdfText(msg.path, msg.maxPages).then(
