#!/usr/bin/env bash
#
# Phase 0 de-risk spike for the Bibliophile AppleScript dictionary.
#
# Proves that an Electron app can expose a NATIVE AppleScript dictionary
# (`tell application "Bibliophile" to ...`) using only:
#   - Info.plist  NSAppleScriptEnabled = true
#   - Info.plist  OSAScriptingDefinition = Bibliophile.sdef
...
# Bibliophile.app for us. macOS only.
#
...
if [ -n "$ok" ]; then
  echo "PASS — Bibliophile AppleScript pipeline works:"
